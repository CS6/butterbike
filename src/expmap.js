...
const LATITUDE = 25.0583033;
const LONGITUDE = 121.534873;
...
export default class App extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			coordinates: [
				{
					latitude: 25.0583033,
					longitude: 121.534873,
				},
				{
					latitude: 25.0468924,
					longitude: 121.5203377,
				}
			],
			// coordinates: [
			// 	{
	...
	call() {
		...
		// let T=this.state.location.coords.latitude
		let DB = { nativeEvent: { latitude: 24.147782, longitude: 120.673492 } }
		this.onMapPress(DB);
	}
	getLocation() {
		Geolocation.getCurrentPosition((location) => {
			let coordinate = [location.coords.longitude, location.coords.latitude]
			let G = location.coords.longitude
			let T = location.coords.latitude
			let data = [
				{ G, T },
				{ latitude: 24.387782, ongitude: 120.673492, }
			]
	...
	onMapPress = (e) => {
		if (this.state.isGetMyLocation == false) {
			if (this.state.coordinates.length == 2) {
				this.setState({
					coordinates: [
		...
		if (Platform.OS === 'android') {
			return (
				<View style={styles.container}>
					<Text>
						{"測試測試 For some reason Android crashes here on Expo, so you'll have to test this with iOS … Sorry"}
					</Text>
				</View>
			);
		}
